...
- Connaître quelques unes des courbes mathématiques les plus utilisées en biologie
...
Afin d'avoir un premier aperçu de ce qu'est une régression non linéaire par les moindres carrés et comment on la calcule dans R, nous allons détailler un exemple concret. La posidonie *Posidonia oceanica* (L.) Delile (1813) est une plante à fleur marine qui forme des herbiers denses en mer Méditerranée. Ses feuilles sont particulièrement adaptées à l'utilisation de la lumière qui règne à quelques mètres en dessous de la surface où elle prospère en herbiers denses.

![[Herbier de posidonies](http://www.futura-sciences.com/planete/dossiers/nature-posidonie-miracle-mediterranee-2006/)](images/sdd2_04/posido.jpg)
...
![[Diving PAM](https://www.walz.com/files/downloads/manuals/diving-pam-II/DIVING-PAM-II-Brochure.pdf)](images/sdd2_04/diving_pam.jpg)

Cet appareil est capable de déterminer le taux de transfert des électrons (ETR en µmol électrons/m^2^/s) par l'analyse de la fluorescence réémise par la plante lorsque ses photosites sont excités par une lumière monochromatique pulsée [@diving_pam2018]. Une façon de déterminer la réponse de la plante en rendement photosynthétique en fonction de l'intensité de la lumière reçue est de mesurer successivement l'ETR pour différentes intensités de lumière. En anglais cela s'appelle la "Rapid Light Curve" ou RLC en abrégé. Comme toutes les longueurs d'ondes lumineuses ne sont pas utilisables par la chlorophylle, l'intensité lumineuse est exprimé dans une unité particulière, le "PAR" ou "Photosynthetically Active Radiation" en µmol photons/m^2^/s. Une RLC représente donc la variation de l'ERT en fonction des PAR^[Techniquement, l'ERT nécessite d'effectuer des mesures après stabilisation de la photosynthèse, ce qui prend plusieurs minutes. Donc, une courbe *rapide* est discutable de ce point de vue mais reste un outil utile en écophysiologie où la comparaison de différentes RLCs indique une changement de capacité photosynthétique de la plante.]. Une RLC typique commence par une relation quasi-linéaire aux faibles intensités, pour s'infléchir et atteindre un plateau de rendement maximum. Au delà, si l'intensité lumineuse augmente encore, des phénomènes de photoinhibition apparaissent et le rendement diminue dans une troisième phase. Voici une RLC mesurée à l'aide du diving PAM sur une feuille de *P. oceanica*.
...
A gauche, nous avons la valeur de la fonction objective $f_{obj}$, et à droite des deux points, la valeur actuelle des trois paramètres dans l'ordre `etr_max`, `alpha` et `beta`. La première ligne indique l'étape 0. $f_{obj}$ vaut 24,3 avec les valeurs par défaut. Dès l'étape suivante (itération #1), cette valeur est divisée par 6 et vaut 4,2. A ce stade, `etr_max = 8,5`, `alpha = 0,29` et `beta = 0,0019`. Les étapes #2 et #3 font encore significativement diminuer $f_{obj}$, et puis nous grappillons des décimales.
...
anim_plot +
  transition_layers(layer_length = 1, transition_length = 0.5,
    from_blank = FALSE, keep_layers = c(Inf, 0, 0, 0, 0, 0, 0, 0, 0),
    layer_names = c("",
      paste("Iteration", 0:7, "- f_obj =", round(params$f_obj, 8))))
anim_save("nls_anim.gif")
```

![](images/sdd2_04/nls_anim.gif)
...
Ensuite, nous recherchons l'aide relative à ces fonctions, par exemple via `?SSasymp`.
...
`r launch_shiny("https://sdd.umons.ac.be/B04Sa_micmen/", height = 600, delay = 20, toc = "Ajuster la meilleure courbe de Michaelis-Menten")`
...
```{r assign_B04Ia_speed_reaction, echo=FALSE, results='asis'}
if (exists("assignment"))
  assignment("B04Ia_speed_reaction", part = NULL,
    url = "https://github.com/BioDataScience-Course/B04Ia_speed_reaction",
    course.ids = c(
      'S-BIOG-015' = "B04Ia_21M_speed_reaction", 
      'S-BIOG-937-958-959' = "B04Ia_21C_speed_reaction"),
    course.urls = c(
      'S-BIOG-015' = "https://classroom.github.com/a/o7PG28iR", 
      'S-BIOG-937-958-959' = "https://classroom.github.com/a/..."),
    course.starts = c(
      'S-BIOG-015' = !"{W[12]+1} 08:00:00"),
    course.ends = c(
      'S-BIOG-015' = !"{W[15]+5} 23:59:59"),
    term = "Q1", level = 3,
    toc = "Vitesse de réaction chimique")
```
...
`r launch_shiny("https://sdd.umons.ac.be/B04Sb_exponent/", height = 600, delay = 20, toc = "Ajuster la meilleure courbe exponentielle")`
...
`r launch_shiny("https://sdd.umons.ac.be/B04Sc_logis/", height = 600, delay = 20, toc = "Ajuster la meilleure courbe logistique")`
...
`r launch_shiny("https://sdd.umons.ac.be/B04Sd_gen_logis/", height = 600, delay = 20, toc = "Ajuster le meilleur modèle logistique généralisé")`
...
`r launch_shiny("https://sdd.umons.ac.be/B04Se_gompertz/", height = 600, delay = 20, toc = "Ajuster le meilleur modèle de Gompertz")`
...
`r launch_shiny("https://sdd.umons.ac.be/B04Sf_richards/", height = 600, delay = 20, toc = "Ajuster le meilleur modèle de Richards")`
...
`r learnr("B04La_nls", title = "Modèles non-linéaires", toc = "Récapitulatif modèles non linéaires")`

```{r assign_B04Gb_bacterial_growth, echo=FALSE, results='asis'}
if (exists("assignment2"))
  assignment2("B04Gb_bacterial_growth", part = NULL,
    url = "https://github.com/BioDataScience-Course/B04Gb_bacterial_growth",
    course.ids = c(
      'S-BIOG-015' = "B04Gb_21M_bacterial_growth", 
      'S-BIOG-937-958-959' = "B04Gb_21C_bacterial_growth"),
    course.urls = c(
      'S-BIOG-015' = "https://classroom.github.com/a/cj9NgLMG", 
      'S-BIOG-937-958-959' = "https://classroom.github.com/a/..."),
    course.starts = c(
      'S-BIOG-015' = !"{W[12]+1} 08:00:00"),
    course.ends = c(
      'S-BIOG-015' = !"{W[15]+5} 23:59:59"),
    term = "Q1", level = 4, n = 2,
    toc = "Croissance bactérienne (par binômes)")
```
...
```{r assign_B04Ca_models, echo=FALSE, results='asis'}
if (exists("challenge"))
  challenge("B04Ca_models", part = NULL,
    url = "https://github.com/BioDataScience-Course/B04Ca_models",
    course.ids = c(
      'S-BIOG-015' = "B04Ca_21M_models"),
    course.urls = c(
      'S-BIOG-015' = "https://classroom.github.com/a/QyWgve1a"),
    course.starts = c(
...
`r launch_shiny("https://sdd.umons.ac.be/B04Sa_models/", height = 800, delay = 20, toc = NULL, alt1 = "*Cliquez pour visualiser le classement.*", alt2 = "*Cliquez pour visualiser le classement.*")`

<!-- <iframe width="780" height="800" frameborder="0" scrolling="auto" class="shiny-app" id=\"B04Sa_models\" src="https://sdd.umons.ac.be/B04Sa_models"></iframe> -->
:::


## Récapitulatif des exercices

Ce quatrième module vous a permis de comprendre les régressions non linéaires. Pour évaluer votre compréhension de cette matière vous aviez les exercices suivants à réaliser\ :
