...
Donc, la principale différence entre ANOVA et régression linéaire telles que nous les avons abordés jusqu'ici réside dans la **nature** de la ou des variables indépendantes, c'est-à-dire, leur type. Pour rappel, il existe deux grandes catégories de variables\ : quantitatives et qualitatives, et deux sous-catégories pour chacune d'elle. Cela donne quatre types principaux de variables, formant plus de 90% des cas rencontrés\ :
...
```{block2, type='warning'}
Par la suite, un encodage correct des variables sera *indispensable* afin de distinguer correctement ces différentes situations. En effet, R considèrera automatiquement comment mener l'analyse en fonction de la classe des variables fournies. Donc, si la classe est incorrecte, l'analyse le sera aussi\ ! Si vous avez des doutes concernant les types de variables, relisez la section [type de variables](https://wp.sciviews.org/sdd-umons/?iframe=wp.sciviews.org/sdd-umons-2021/types-de-variables.html) avant de continuer ici. 
```

## ANOVA et régression linéaire

Avez-vous remarqué une ressemblance particulière entre la régression linéaire que nous avons réalisé précédemment et l'analyse de variance (ANOVA)\ ? Les plus observateurs auront mis en avant que la fonction de base dans R est la même dans les deux cas\ : `lm()`. Cette fonction est donc capable de traiter aussi bien des variables réponses qualitatives que quantitatives, et effectue alors une ANOVA dans un cas ou une régression linéaire dans l'autre.
...
Le calcul est, en réalité, identique en interne. Il est donc possible de généraliser ces deux approches en une seule appelée **modèle linéaire**, mais à condition d'utiliser une astuce pour modifier nos modèles afin qu'ils soient inter-compatibles.
...
avec $I_i$, une variable dite **indicatrice** créée de toute pièce qui prend la valeur 1 lorsque le niveau correspond à _i_, et 0 dans tous les autres cas. 
...
Dans le cas de notre version simplifiée du modèle mathématique où nous avons fait disparaître $I_1$ en l'assimilant à la moyenne $\mu$ pour obtenir $\beta_1$. Dans le cas où notre variable qualitative a quatre niveaux, nous avons donc le modèle suivant\ :
...
moyenne des niveaux précédents. La matrice de contrastes correspondant pour une variable à quatre niveaux s'obtient à l'aide de la fonction R `contr.helmert()`\ :
...
```{r echo=FALSE}
tibble(X = c(0, 3, 0, 3), Y = c(2, 5, 3, 6), F = c("A", "A", "B", "B")) %>.%
  chart(data = ., Y ~ X %col=% F) +
    geom_line()
```

- Les deux nuages de points s'allongent linéairement le long de **droites ayant des pentes différentes mais qui se croisent en _X_ = 0**. Les droites diffèrent donc **par leurs pentes** mais pas par leurs ordonnées à l'origine. Cela s'exprime par des **interactions entre _X_ et _F_** dans le modèle qui se notent `X:F` dans une formule. Nous écrirons donc notre modèle comme `Y = X + X:F`.

```{r echo=FALSE}
tibble(X = c(0, 3, 0, 3), Y = c(2, 5, 2, 6), F = c("A", "A", "B", "B")) %>.%
  chart(data = ., Y ~ X %col=% F) +
    geom_line()
```

- Les deux droites se croisent en un point quelconque différent de _X_ = 0. Les droites diffèrent *à la fois* par leurs pentes et par leurs ordonnées à l'origine. Le **modèle complet** sera alors utilisé. Dans R nous écrirons\ : `Y = X + F + X:F`. Une forme abrégée synonyme existe aussi\ : `Y = X * F`. Ces deux formules décrivent strictement le même modèle et sont interchangeables.

```{r echo=FALSE}
tibble(X = c(0, 3, 0, 3), Y = c(2, 6, 3, 5), F = c("A", "A", "B", "B")) %>.%
  chart(data = ., Y ~ X %col=% F) +
    geom_line()
```
...
if (exists("assignment2"))
  assignment2("B02Ga_urchin", part = "II",
    url = "https://github.com/BioDataScience-Course/B02Ga_urchin",
    course.ids = c(
      'S-BIOG-015' = "B02Ga_21M_urchin", 
      'S-BIOG-937-958-959' = "B02Ga_21C_urchin"),
    course.urls = c(
      'S-BIOG-015' = "ttps://classroom.github.com/a/wqol1C5h", 
      'S-BIOG-937-958-959' = "https://classroom.github.com/g/..."),
    course.starts = c(
      'S-BIOG-015' = "2021-11-15 13:00:00"),
    course.ends = c(
      'S-BIOG-015' = "2021-11-29 23:59:59"),
    toc = "Exercices pratiques sur le modèle linéaire avec urchin",
    n = 2)
```
...
if (exists("assignment"))
  assignment("B03Ia_ovocyte", part = NULL,
    ...
    course.ids = c(
      'S-BIOG-015' = "B03Ia_21M_ovocyte", 
      'S-BIOG-937-958-959' = "B03Ia_21C_ovocyte"),
    course.urls = c(
    ...
    course.ends = c(
      'S-BIOG-015' = "2021-11-29 23:59:59"),
    toc = "Exercices pratiques cadrés sur le modèle linéaire généralisé avec ovocyte")
```

```{r, echo=FALSE, results='asis'}
if (exists("assignment2"))
  assignment2("B03Gb_human", part = NULL,
    url = "https://github.com/BioDataScience-Course/B03Gb_human",
    course.ids = c(
      'S-BIOG-015' = "B03Gb_21M_human", 
      'S-BIOG-937-958-959' = "B03Gb_21C_human"),
    course.urls = c(
      'S-BIOG-015' = "https://classroom.github.com/a/UDiu0W69", 
      'S-BIOG-937-958-959' = "https://classroom.github.com/a/..."),
    course.starts = c(
      'S-BIOG-015' = "2021-11-15 13:00:00"),
    course.ends = c(
      'S-BIOG-015' = "2021-11-29 23:59:59"),
    toc = "Exercices libres par binome sur le modèle linéaire (généralisé) avec human",
    n = 2)
```

## Récapitulatif des exercices

Ce troisième module vous a permis de comprendre le modèle linéaire, de découvrir l'ANCOVA ainsi qu'apprendre le mécanisme du modèle linéaire généralisé. Pour évaluer votre compréhension de cette matière vous aviez les exercices suivants à réaliser\ :
